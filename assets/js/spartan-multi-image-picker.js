...
(function ($) {
    ...
    $.fn.spartanMultiImagePicker = function(options) {
        ...
        var defaults = {
            ...
            onSizeErr:         function() {},
            directUpload :  {
                loaderIcon: '<i class="fa fa-sync fa-spin"></i>',
                status:       false,
                url:          '',
                success:      function() {},
                error:        function() {}
        ...
        function addRow(settings, el){
            last_index = count;
            var groupClassName = settings.groupClassName, rowHeight = settings.rowHeight, fieldName = settings.fieldName, fieldRequired=settings.fieldRequired, placeholderImage = settings.placeholderImage, dropFileLabel = settings.dropFileLabel;
            var placeholderImageTarget = placeholderImage.image;
            var placeholderImageWidth  = '64px';

            var uploadLoaderIcon = '<i class="fa fa-sync fa-spin"></i>';
            ...
            var template = `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 20px; ">`+
                                `<div style="position: relative;">`+
                                    ...
                                    `<label class="file_upload" style="width: 100%; height: ${rowHeight}; border: 2px dashed #ddd; border-radius: 3px; cursor: pointer; text-align: center; overflow: hidden; padding: 5px; margin-top: 5px; margin-bottom : 5px; position : relative; display: flex; align-items: center; margin: auto; justify-content: center; flex-direction: column;">`+
                                        `<a href="javascript:void(0)" data-spartanindexremove="${count}" style="position: absolute !important; right : 0px; top: 0px; display : none; background : #FED700; border-radius: 3px; width: 20px; height: 20px; line-height : 20px; text-align: center; text-decoration : none; color : #313E46;" class="spartan_remove_row"><i class="fa fa-times"></i></a>`+
                                        ...
                                        `<input class="form-control spartan_image_input" accept="image/*" data-spartanindexinput="${count}" style="display : none"  name="${fieldName}" ${fieldRequired} type="file">`+
                                   `</label> `+
                                `</div>`+
                           `</div>`;
